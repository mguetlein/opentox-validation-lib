  ...
  module ValidationLib
    ...
    class Predictions
      ...
      def initialize(test_dataset_uri, prediction_dataset_uri)
        ...
        test_dataset = Dataset.find(:uri => test_dataset_uri)
        prediction_dataset = Dataset.find(:uri => prediction_dataset_uri)
        ...
        test_dataset.compounds.each do |c|
          ...
          {prediction_dataset => @predicted_values, test_dataset => @actual_values}.each do |d, v|
            d.features(c).each do |a|
              case Feature.new(:uri => a.uri).value('classification').to_s
              when 'true'
                v.push(1.0)  
              when 'false'
                v.push(0.0)
          ...
          prediction_dataset.features(c).each do |a|
            @confidence_values.push Feature.new(:uri => a.uri).value('confidence').to_f
          end
        end
